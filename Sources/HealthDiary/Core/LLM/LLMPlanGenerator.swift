...
final class LLMPlanGenerator {
    ...
    private func buildPrompt(request: GenerationRequest, recipes: [RecipeRecord]) -> String {
        ...
        // レシピ行を短く（トークン削減でメモリ節約）
        let recipeLines = recipes.map { r in
            "\(r.id)|\(r.name)|\(Int(r.caloriesPerServing))kcal"
        }.joined(separator: "\n")
        let slotsText = buildSlotsText(request: request)

        return """
        家族向け献立をJSONで出力してください。
        ...
        JSON形式で出力（説明文不要、コードブロック不要）:
        {"days":[{"date":"YYYY-MM-DD","meals":{"breakfast":{"recipe_id":"ID","recipe_name":"名前"},"lunch":{"recipe_id":"ID","recipe_name":"名前"},"dinner":{"recipe_id":"ID","recipe_name":"名前"}}}]}
        スロット外のキーは省略。recipe_idはリスト内のIDのみ。
        """
    }
    ...
    private func buildDayPrompt(
        ...
        return """
        \(dateStr)(\(slotStr))の献立をJSONで出力してください。
        家族: \(familyText)
        使用済み（重複不可）: \(otherMeals.isEmpty ? "なし" : otherMeals)
        レシピ一覧（ID|名前|カロリー）:
        \(recipeLines)
        JSON形式（説明文不要）:
        {"date":"\(dateStr)","meals":{"breakfast":{"recipe_id":"ID","recipe_name":"名前"},"dinner":{"recipe_id":"ID","recipe_name":"名前"}}}
        スロット外のキーは省略。
        """
    }
