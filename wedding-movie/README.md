...
書き出し設定は `scripts/render.mjs` にまとめてあります。指定が違えばここを変えてください。

| 設定 | 値 | 理由 |
|---|---|---|
| コーデック | H.264 | 会場のプレイヤーで最も確実に再生できる |
| ピクセルフォーマット | yuv420p | 同上。互換性優先 |
| 色空間 | bt709（テレビレンジ） | **これが無いとフルレンジ(`yuvj420p`)で出てしまい、プロジェクタによっては色が浅く、または黒が潰れて見えます** |
| CRF | 18 | 実質見た目が変わらない範囲で最高画質寄り |
| 音声 | 無音のAACトラック | 中身は無音だが、トラック自体は残している（音声トラックが無いファイルで、会場のプレイヤーやDVDオーサリングがつまずくことがあるため）。不要なら `npm run render -- --muted` |
